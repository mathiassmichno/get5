...
stock bool LoadMatchFromUrl(const char[] url, ArrayList paramNames = null,
  ...
  if (ripextAvaliable) {
    g_HTTPClient = new HTTPClient(cleanedUrl);
    if (g_HTTPClient == INVALID_HANDLE) {
      MatchConfigFail("Failed to create HTTP GET request");
      return false;
    }

    g_HTTPClient.Get("", OnMatchConfigReceived);
    return true;
...
// SteamWorks HTTP callback fintor fetching a workshop collection
